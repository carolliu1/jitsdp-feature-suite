"""Kotlin parser using language-tuned generic tree-sitter extraction."""
...
class KotlinParser(GenericTreeSitterParser):
    """Kotlin-specific parser wrapper."""

    def __init__(self) -> None:
        super().__init__(
            profile=LANGUAGE_PROFILES["kotlin"],
            import_patterns=(r"^\s*import\s+([a-zA-Z0-9_.]+)",),
            receiver_type_patterns=(r"\b(?:val|var)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*:\s*([A-Z][A-Za-z0-9_<>]*)",),
        )
